...
func ContractSignersFromContext(ctx context.Context) []AccountID {
	return ctx.Value(contractSignersContextKey{}).([]AccountID)
}

// ContextWithContractSigners returns a context with an updated accounts
func ContextWithContractSigners(ctx context.Context, accounts []AccountID) context.Context {
