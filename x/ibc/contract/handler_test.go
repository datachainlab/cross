...
func (suite *HandlerTestSuite) TestHandleContractCall() {
	...
	{
		{
			msg := contract.NewMsgContractCall(acc0, []sdk.AccAddress{acc0}, nil, cross.ExactStateCondition)
...

		{
			...
			msg := contract.NewMsgContractCall(acc0, []sdk.AccAddress{acc0}, bz, cross.ExactStateCondition)
	...
	// Ensure that commit store state successfully
	{
...

		{
			...
			bz, err := contract.EncodeContractSignature(c)
			suite.Require().NoError(err)
			msg := contract.NewMsgContractCall(acc1, []sdk.AccAddress{acc0}, bz, cross.ExactStateCondition)
			ctx, _ := suite.ctx.CacheContext()
